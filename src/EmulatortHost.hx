...
class EmulatortHost
{
  ...
  var display : CanvasRenderingContext2D;
	...
	var testProgram = haxe.Resource.getString("testcode");
    ...
    display = displayCanvas.getContext2d();
		...
		runButton.onclick = function() { halted = !halted; runButton.textContent = halted?"Run":"Stop"; };
		...
		for (i in 0...10) {
			trace(StringTools.hex(avr.progMem[i],4));
		}
		/* 
		// first test program 
		...
		*/
		...
		avr.outPortFunctions[0x22] = function (value) {
		...
		avr.outPortFunctions[displayPort + 0x0f] = function (value) {
			switch (value) {
			case 0:
		...
		avr.outPortFunctions[displayPort + 0x00] = function (value) {			
			displayGenerator.colorData_displayStart &= 0xff00;
			displayGenerator.colorData_displayStart |= value & 0x00ff;
		}
		avr.outPortFunctions[displayPort+0x01] = function (value) {
			displayGenerator.colorData_displayStart &= 0x00ff;
			displayGenerator.colorData_displayStart |= (value & 0x00ff) << 8;
		}
		avr.outPortFunctions[displayPort + 0x02] = function (value) {			
			displayGenerator.pixelData_displayStart &= 0xff00;
			displayGenerator.colorData_displayStart |= value & 0x00ff;
		}
		avr.outPortFunctions[displayPort+0x03] = function (value) {
			displayGenerator.pixelData_displayStart &= 0x00ff;
			displayGenerator.pixelData_displayStart |= (value & 0x00ff) << 8;
		}

		avr.outPortFunctions[displayPort + 0x04] = function (value) {
		  displayGenerator.pixelData_increment = value & 0xff;
		}

		avr.outPortFunctions[displayPort + 0x05] = function (value) {
		  displayGenerator.colorData_increment = value & 0xff;
		}

		avr.outPortFunctions[displayPort + 0x06] = function (value) {
		  displayGenerator.pixelData_lineIncrement = (value & 0xff)<<3;
		}

		avr.outPortFunctions[displayPort + 0x07] = function (value) {
		  displayGenerator.colorData_lineIncrement = (value & 0xff)<<3;
		}

		avr.outPortFunctions[displayPort + 0x08] = function (value) {
		  displayGenerator.displayShiftX = (value >> 4) & 0xf;
		  displayGenerator.displayShiftY = value & 0xf;
		}

		avr.outPortFunctions[displayPort + 0x09] = function (value) {
		  displayGenerator.serialPixelAddress = (displayGenerator.serialPixelAddress & 0xffff00) | (value & 0xff);
		}
		avr.outPortFunctions[displayPort + 0x0A] = function (value) {
		  displayGenerator.serialPixelAddress = (displayGenerator.serialPixelAddress & 0xff00ff) | ((value & 0xff) << 8);
		}
		avr.outPortFunctions[displayPort + 0x0B] = function (value) {

		  displayGenerator.serialPixelAddress = (displayGenerator.serialPixelAddress & 0x00ffff) | ((value & 0xff) <<16);
		}

		avr.outPortFunctions[displayPort + 0x0C] = function (value) {
			displayGenerator.serialSet(value);			
		}
		avr.outPortFunctions[displayPort + 0x0D] = function (value) {
			displayGenerator.serialMul(value);			
		}
		avr.outPortFunctions[displayPort + 0x0E] = function (value) {
		...
		var secondTimer = new Timer(500);
		var lastTime = avr.clockCycleCount;
		secondTimer.run = function() { 
	...
	public function updateRegisterView() {
		...
		sregDiv.textContent	= 'SREG: ${StringTools.hex(avr.SREG,2)}';
