...
#define mark(x)            (car(x) = (object *)(((uint16_t)(car(x))) | 0x8000))
#define unmark(x)          (car(x) = (object *)(((uint16_t)(car(x))) & 0x7FFF))
...
#define workspacesize  315
#elif defined(__AVR_ATmega32U4__)
#define workspacesize   421
#elif defined(__AVR_ATmega2560__)
#define workspacesize   1461
#elif defined(__AVR_ATmega1284P__)
#define workspacesize   3000
#else
#define workspacesize   315
#endif

// Length of longest symbol + 1
#define buflen 13
...
ODDP, EVENP, READ, EVAL, LOCALS, GLOBALS, MAKUNBOUND, BREAK, PRINT, PRINC, GC,

#ifdef Arduino_hardware
//PINMODE, DIGITALREAD, DIGITALWRITE,ANALOGREAD, ANALOGWRITE, DELAY, MILLIS, SHIFTOUT, SHIFTIN, NOTE,
#endif
...
int16_t ReturnFlag = 0;
object *freelist;

object *GlobalEnv;
object *GCStack = NULL;
char buffer[buflen+1];
int16_t BreakLevel = 0;
...
char *lookupstring (uint16_t name);
int16_t lookupfn(unsigned int name);
int16_t builtin(char* n);
...
void initworkspace () {
  freelist = NULL;
  for (uint16_t i=0; i<workspacesize; i++) {
...
uint16_t random_below(uint16_t below)   {
   uint32_t result = random_uint16() * below;
   return result >> 16;
}
...
int16_t integer(object *obj){
  if(obj->type != NUMBER) error(F("not a number"));
  return obj->integer;
}

int16_t issymbol(object *obj, unsigned int n) {
  return obj->type == SYMBOL && obj->name == n;
}

int16_t eq (object *arg1, object *arg2) {
  int16_t same_object = (arg1 == arg2);
  int16_t same_symbol = (arg1->type == SYMBOL && arg2->type == SYMBOL && arg1->name == arg2->name);
  int16_t same_number = (arg1->type == NUMBER && arg2->type == NUMBER && arg1->integer == arg2->integer);
  return (same_object || same_symbol || same_number);
}

// Lookup variable in environment

object *value(uint16_t n, object *env) {
  while (env != NULL) {
    object *item = car(env);
...
object *closure (int16_t tail, object *fname, object *function, object *args, object **env) {
...
inline int16_t listlength (object *list) {
  int16_t length = 0;
  while (list != NULL) {
    list = cdr(list);
    length++;
...
object *apply (object *function, object *args, object **env) {
  if (function->type == SYMBOL) {
    uint16_t name = function->name;
    int16_t nargs = listlength(args);
...
object *sp_incf (object *args, object *env) {
  ...
  int16_t result = integer(eval(var, env));
  int16_t temp = 1;
...
object *sp_dotimes (object *args, object *env) {
  object *params = first(args);
  object *var = first(params);
  int16_t count = integer(eval(second(params), env));
  int16_t index = 0;
  object *result = third(params);
...
object *sp_formillis (object *args, object *env) {
  error(F("'formillis' not available"));
...
object *fn_random (object *args, object *env) {
  ...
  return number(random_below(arg));
}
...
object *fn_pinmode (object *args, object *env) {
  print_rom(F("pinmode not available\n"));
/*
  (void) env;
...
object *fn_digitalread (object *args, object *env) {
  print_rom(F("digitalread not available\n"));
/*
  (void) env;
  int pin = integer(first(args));
...
object *fn_digitalwrite (object *args, object *env) {
  print_rom(F("digitalwrite not available\n"));
...
object *fn_analogread (object *args, object *env) {
  error(F("'analogread' not available"));
/*
  (void) env;
  int pin = integer(first(args));
#if defined(__AVR_ATmega328P__)
...
object *fn_analogwrite (object *args, object *env) {
  error(F("'analogwrite' not available"));
  /*
  (void) env;
  int pin = integer(first(args));
#if defined(__AVR_ATmega328P__)
...
object *fn_shiftout (object *args, object *env) {
  error(F("'shiftout' not available"));
...
object *fn_moveto (object *args, object *env) {
  (void) env;
  object* point = first(args);
  int16_t x = integer(first(point));
  int16_t y = integer(second(point));
  pen_x=x;
  pen_y=y;
  return nil;
}

object *fn_lineto (object *args, object *env) {
  (void) env;
  object* point = first(args);
  int16_t x = integer(first(point));
  int16_t y = integer(second(point));
  line(pen_x,pen_y,x,y,pen_color);
  pen_x=x;
  pen_y=y;
  return nil;
}
object *fn_color (object *args, object *env) {
  (void) env;
    int16_t color = integer(first(args));
    pen_color=color&0x0f;
  return nil;
}
...
#endif
const char string112[] PROGMEM = "moveto";
const char string113[] PROGMEM = "lineto";
const char string114[] PROGMEM = "color";
...
#endif
{ string112, fn_moveto, 1, 1 },
{ string113, fn_lineto, 1, 1 },
{ string114, fn_color, 1, 1 },
};
...
void printobject(object *form){
  if (form == NULL) print_rom(F("nil"));
    ...
    print_int16(integer(form));
  } else if (form->type == SYMBOL){
    print(name(form));
    #if defined(debug2)
    print_char('[');print_uint16()(int)form);print_char(']');
    #endif
  } else
...
object *nextitem() {
  ...
  //uint8_t hold = PORT_MOUSEX; //marker for finding code in objdump
...
void repl(object *env) {
  ...
  for (;;) {
    ...
    //randomSeed(micros());
    gc(NULL, env);
    print_uint16(freespace);
    if (BreakLevel) {
      print_rom(F(" : "));
      print_int16(BreakLevel);
    }
    print_rom(F("> "));
    show();
    object *line = read();
